...
// HitechMap.tsx / StreetlightsMap.tsx.
setOptions({ key: process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || '', v: 'weekly' })

/* ── Component ─────────────────────────────────────────────── */
export default function RoadAssetsMap({ project, section, onLoadStats }: Props) {
  ...
  const filterKey = `${project}|${section}`

  /* ── Load data from API ──────────────────────────────────
     Same coarse-then-refined pattern as StreetlightsMap: a fresh filter
     (project/section change) blocks with `loading`; a viewport-only refresh
     (idle firing after a pan/zoom) just sets `refreshing`, no full-screen
     overlay. ── */
  useEffect(() => {
    const isFreshFilter = prevFilterRef.current !== filterKey
    ...
    fetch(`/api/road-assets?${params.toString()}`)
      .then(r => r.json())
      .then(d => {
      ...
      .catch(() => { setError('Failed to load road assets data'); setLoading(false); setRefreshing(false) })
      ...
      .then(({ Map }) => {
        ...
        try {
          localMap = new Map(mapContainer.current, {
            center: { lat: 9.0, lng: 6.0 }, // Nigeria-wide default — spans Kebbi (north) to Coastal Road/Calabar (south)
            zoom: 6,
            mapTypeId: 'hybrid',
  ...
  useEffect(() => {
    ...
    const markers = clusters.map(c => {
      const isSingle = c.count === 1 && c.id != null
    ...
    // fitBounds only on a real filter change, never on a viewport-driven
    // refresh — same lastFitKeyRef guard as HitechMap/StreetlightsMap,
    // prevents an infinite fitBounds -> idle -> refetch loop.
    if (clusters.length > 0 && lastFitKeyRef.current !== filterKey) {
      lastFitKeyRef.current = filterKey
      const bounds = new google.maps.LatLngBounds()
      clusters.forEach(c => bounds.extend({ lat: c.lat, lng: c.lng }))
      map.fitBounds(bounds, 60)
    }
  }, [mapLoaded, clusters, D, filterKey])
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      <div className="ra-map-frame" style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', height: 520, boxShadow: SH.well }}>
        ...
        {(loading || !mapLoaded) && (
          <div style={{ position: 'absolute', inset: 0, background: `${D.bg}e0`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, zIndex: 10 }}>
            <div style={{ width: 32, height: 32, border: `3px solid ${D.sub}`, borderTop: `3px solid ${D.amber}`, borderRadius: '50%', animation: 'raSpin 0.8s linear infinite' }} />
            <span style={{ color: D.muted, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 1 }}>
              {loading ? 'LOADING ROAD ASSETS…' : 'INITIALISING MAP…'}
            </span>
          </div>
        )}
        ...
        {selCell && (
          <div style={{ position: 'absolute', top: 12, left: 12, zIndex: 20, background: D.panel2, border: `1px solid ${D.amber}55`, borderRadius: 10, padding: '14px 16px', minWidth: 220, maxWidth: 280, boxShadow: SH.cardLg }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {selCell.entityType && <InfoRow D={D} label="Type" value={selCell.entityType} />}
              {selCell.project && <InfoRow D={D} label="Project" value={selCell.project} />}
              ...
              {selCell.id != null && <InfoRow D={D} label="ID" value={selCell.id} />}
              {selCell.count > 1 && (
      ...
      <style>{`
        @media (max-width: 640px) { .ra-map-frame { height: 360px !important; } }
