  ...
  return (
    <a href={href} title={label} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        ...
        color: isActive ? (isLight ? '#fff8ea' : '#0e0e10') : hov ? D.amber : D.sub,
        background: isActive ? D.amber : hov ? `${D.amber}1a` : 'transparent',
        boxShadow: isActive ? `0 0 18px ${D.amber}59, inset 0 1px 0 rgba(255,255,255,${isLight ? 0.35 : 0.2})` : 'none',
...
export default function SideNav() {
  ...
  return (
    <nav className="side-nav" style={{
      ...
      width: 64, flexShrink: 0, background: isLight ? '#ffffff' : '#161618',
      borderRight: `1px solid ${D.border}`,
      boxShadow: isLight ? '2px 0 12px rgba(15,23,42,0.06)' : '2px 0 12px rgba(0,0,0,0.4)',
      ...
      {NAV_ITEMS.map(item => {
        ...
        return <NavItem key={item.href} href={item.href} label={item.label} Icon={item.icon} isActive={isActive} D={D} isLight={isLight}/>
      })}
