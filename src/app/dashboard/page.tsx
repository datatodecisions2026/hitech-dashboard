...
interface DashData {
  ...
  activeFilters: { filterCategory: string; filterProject: string; filterDateFrom: string; filterDateTo: string; filterChFrom: string; filterChTo: string }
}
  ...
  return (
    <div style={{ display: 'flex', gap: 28, alignItems: 'center', flexWrap: 'wrap' }}>
      <svg width={190} height={190} viewBox="-95 -95 190 190" style={{ flexShrink: 0 }} onMouseLeave={() => setHov(null)}>
        ...
        {segments.map((seg, i) => {
          const isHov = hov === i
          return <circle key={i} r={r} fill="none" stroke={seg.color} strokeWidth={isHov ? sw + 6 : sw}
            strokeDasharray={`${ready ? seg.len : 0} ${circ}`} strokeDashoffset={-(seg.offset)} strokeLinecap="butt"
            strokeOpacity={hov !== null && !isHov ? 0.22 : 1}
            style={{ transition: `stroke-dasharray 0.8s cubic-bezier(0.16,1,0.3,1) ${i * 0.07}s, stroke-width 0.2s, stroke-opacity 0.2s`, cursor: 'pointer', filter: isHov ? `drop-shadow(0 0 6px ${seg.color}88)` : 'none' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, flex: 1, minWidth: 140 }}>
        {segments.map((seg, i) => {
          const isHov = hov === i
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', opacity: hov !== null && !isHov ? 0.3 : 1, transition: 'opacity 0.2s' }} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}>
              <div style={{ width: 8, height: 8, borderRadius: 2, background: seg.color, flexShrink: 0, transform: isHov ? 'scale(1.4)' : 'scale(1)', boxShadow: isHov ? `0 0 8px ${seg.color}` : 'none', transition: 'transform 0.25s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.2s' }} />
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto' }}>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block', minWidth: 320 }}>
        ...
        {gridLines.map(v => { const y = padT + chartH - (v / maxVal) * chartH; return <g key={v}><line x1={padL} y1={y} x2={W-padR} y2={y} stroke="rgba(255,255,255,0.04)" strokeWidth={1}/><text x={padL-4} y={y+3} textAnchor="end" fill={D.sub} fontSize="7" fontFamily="var(--font-mono)">{v}</text></g> })}
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={d.name} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}
            style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: hov !== null && !isHov ? 0.35 : 1, transition: 'opacity 0.2s' }}>
            ...
            <div style={{ flex: 1, height: 5, background: 'rgba(255,255,255,0.04)', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
              <div style={{ position: 'absolute', inset: 0, right: 'auto', width: ready ? `${barPct}%` : '0%', background: isTop ? `linear-gradient(90deg, ${color}, ${color}bb)` : `linear-gradient(90deg, ${color}66, ${color}33)`, borderRadius: 6, transition: `width 0.9s cubic-bezier(0.16,1,0.3,1) ${i*0.04}s`, boxShadow: isTop && isHov ? `0 0 8px ${color}55` : 'none' }} />
        ...
        return (
          <div key={d.name}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <span style={{ fontSize: '0.72rem', color: D.muted, fontFamily: 'var(--font-mono)', display: 'flex', gap: 7, alignItems: 'center' }}><span>{WEATHER_ICON[d.name]||'🌡'}</span><span>{d.name}</span></span>
            ...
            <div style={{ height: 4, background: 'rgba(255,255,255,0.04)', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: ready ? `${pct}%` : '0%', background: `linear-gradient(90deg, ${D.blue}, ${D.blue}66)`, borderRadius: 3, transition: `width 0.8s cubic-bezier(0.16,1,0.3,1) ${i*0.08}s` }} />
            </div>
          </div>
        )
      })}
...
function FilterBar({ data, onFilter }: { data: DashData; onFilter: (key: string, val: string) => void }) {
  ...
  return (
    <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'flex-end', padding:'16px 20px', background:D.panel, border:`1px solid ${D.border}`, borderRadius:14, marginBottom:24, boxShadow:SH_PANEL }}>
      ...
      {hasFilters && <button onClick={() => { setChFrom(''); setChTo(''); onFilter('__clear__','') }}
...
function DashSkeleton() {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:14 }}>{[0,1,2,3,4].map(i=><Skel key={i} h={108}/>)}</div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr',   gap:14 }}><Skel h={250}/><Skel h={250}/></div>
      <div style={{ display:'grid', gridTemplateColumns:'3fr 2fr',   gap:14 }}><Skel h={210}/><Skel h={210}/></div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr',   gap:14 }}><Skel h={190}/><Skel h={190}/></div>
...
function DashboardPageInner() {
  ...
  function handleFilter(key: string, val: string) {
    ...
    if (key === '__clear__') {
      ['category','project','date_from','date_to','ch_from','ch_to'].forEach(k => p.delete(k))
  ...
  return (
    <div style={{ minHeight:'100vh', background:D.bg, color:D.text, fontFamily:'var(--font-dm-sans)', backgroundImage:'linear-gradient(rgba(255,255,255,0.012) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.012) 1px, transparent 1px)', backgroundSize:'60px 60px', position:'relative' }}>
      ...
      {/* Sub-header */}
      <div style={{ position:'sticky', top:52, zIndex:50, background:'rgba(14,14,16,0.95)', backdropFilter:'blur(12px)', borderBottom:`1px solid ${D.border}`, padding:'0 32px', display:'flex', alignItems:'center', gap:18, height:46, overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'baseline', gap:10 }}>
          ...
          <span style={{ fontFamily:'var(--font-mono)', fontSize:'0.52rem', letterSpacing:'0.14em', color:D.sub, textTransform:'uppercase', background:D.panel, padding:'2px 8px', borderRadius:4, border:`1px solid ${D.border}` }}>Site Command</span>
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginLeft:'auto' }}>
          ...
          <div style={{ fontFamily:'var(--font-mono)', fontSize:'0.55rem', color:D.sub, letterSpacing:'0.06em' }}>{new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</div>
        </div>
      </div>
