...
const CAT_COLORS = ['#d4a040', '#e87040', '#60a5fa', '#34d399', '#a78bfa', '#e31c3d', '#f472b6']
...
interface DashData {
  ...
  activeFilters: {
    filterCategory: string; filterProject: string
    filterDateFrom: string; filterDateTo: string
    filterChFrom: string;   filterChTo: string
  }
}
  ...
  return (
    <div style={{ display: 'flex', gap: 28, alignItems: 'center', flexWrap: 'wrap' }}>
      <svg width={190} height={190} viewBox="-95 -95 190 190" style={{ flexShrink: 0 }} onMouseLeave={() => setHov(null)}>
        ...
        {segments.map((seg, i) => {
          const isHov = hov === i
          return <circle key={i} r={r} fill="none" stroke={seg.color} strokeWidth={isHov ? sw + 5 : sw} strokeDasharray={`${ready ? seg.len : 0} ${circ}`} strokeDashoffset={-(seg.offset)} strokeLinecap="butt" strokeOpacity={hov !== null && !isHov ? 0.28 : 1} style={{ transition: `stroke-dasharray 0.7s cubic-bezier(0.4,0,0.2,1) ${i * 0.07}s, stroke-width 0.2s ease, stroke-opacity 0.2s ease`, cursor: 'pointer' }} onMouseEnter={() => setHov(i)} />
        })}
        {hovSeg ? (<>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, flex: 1, minWidth: 140 }}>
        {segments.map((seg, i) => {
          const isHov = hov === i
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', opacity: hov !== null && !isHov ? 0.35 : 1, transition: 'opacity 0.2s' }} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)}>
              ...
              <span style={{ fontSize: '0.72rem', color: isHov ? D.text : D.muted, flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: 'var(--font-mono)', transition: 'color 0.2s' }}>{seg.name}</span>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', overflowX: 'auto' }}>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: 'block', minWidth: 320 }}>
        ...
        {gridLines.map(v => { const y = padT + chartH - (v / maxVal) * chartH; return <g key={v}><line x1={padL} y1={y} x2={W - padR} y2={y} stroke="rgba(255,255,255,0.05)" strokeWidth={1} /><text x={padL - 4} y={y + 3} textAnchor="end" fill={D.sub} fontSize="7" fontFamily="var(--font-mono)">{v}</text></g> })}
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.date} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} style={{ cursor: 'default' }}>
              ...
              {isHov && d.count > 0 && (() => { const tx = Math.min(Math.max(x - 18, padL), W - padR - 60); const ty = Math.max(padT + 2, y - 26); return <g><rect x={tx} y={ty} width={60} height={18} rx={4} fill="rgba(20,16,10,0.95)" stroke="rgba(255,255,255,0.12)" strokeWidth={1} /><text x={tx + 30} y={ty + 13} textAnchor="middle" fill={D.text} fontSize="8.5" fontFamily="var(--font-mono)">{fmtDate(d.date)}: {d.count}</text></g> })()}
            </g>
          )
        })}
        {data.filter((_, i) => i % 5 === 0 || i === data.length - 1).map(d => { const i = data.indexOf(d); const x = padL + i * step + step / 2; return <text key={d.date} x={x} y={H - 4} textAnchor="middle" fill={D.sub} fontSize="7.5" fontFamily="var(--font-mono)">{fmtDate(d.date)}</text> })}
      </svg>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: '100%' }}>
      {data.map((d, i) => {
        ...
        return (
          <div key={d.name} onMouseEnter={() => setHov(i)} onMouseLeave={() => setHov(null)} style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: hov !== null && !isHov ? 0.45 : 1, transition: 'opacity 0.2s' }}>
            <div style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isTop ? `${color}22` : 'transparent', border: isTop ? `1px solid ${color}44` : '1px solid transparent', fontSize: 9, fontFamily: 'var(--font-mono)', color: isTop ? color : D.sub, fontWeight: isTop ? 700 : 400 }}>{i + 1}</div>
            <span style={{ width: 140, fontSize: '0.7rem', color: isHov ? D.text : D.muted, fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 0, transition: 'color 0.2s' }} title={d.name}>{d.name}</span>
            <div style={{ flex: 1, height: 6, background: 'rgba(255,255,255,0.04)', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
              <div style={{ position: 'absolute', inset: 0, right: 'auto', width: ready ? `${barPct}%` : '0%', background: isTop ? `linear-gradient(90deg, ${color}, ${color}bb)` : `linear-gradient(90deg, ${color}88, ${color}44)`, borderRadius: 6, transition: `width 0.8s cubic-bezier(0.4,0,0.2,1) ${i * 0.04}s`, boxShadow: isTop && isHov ? `0 0 8px ${color}66` : 'none' }} />
              {isTop && <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.08) 50%, transparent 100%)', animation: 'shimmerSlide 2.5s ease-in-out infinite', animationDelay: `${i * 0.4}s` }} />}
            </div>
            <span style={{ width: 32, textAlign: 'right', fontSize: '0.7rem', color: isHov ? color : D.text, fontWeight: 700, fontFamily: 'var(--font-mono)', flexShrink: 0, transition: 'color 0.2s' }}>{d.count}</span>
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ background: hov ? '#2d2d31' : '#272729', borderRadius: 12, padding: '18px 20px', position: 'relative', overflow: 'hidden', opacity: vis ? 1 : 0, transform: vis ? 'translateY(0)' : 'translateY(10px)', transition: 'opacity 0.5s ease, transform 0.5s ease, box-shadow 0.25s ease, background 0.2s ease', boxShadow: hov ? SH_RAISED_LG : SH_RAISED }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        ...
        {sub && <span style={{ fontSize: '0.6rem', color: D.green, fontFamily: 'var(--font-mono)', background: '#1e1e22', padding: '2px 7px', borderRadius: 4, letterSpacing: '0.06em', boxShadow: 'inset 1px 1px 3px rgba(0,0,0,0.7)' }}>{sub}</span>}
      </div>
...
function MediaGallery({ items, activeProject }: { items: MediaItem[]; activeProject: string }) {
  ...
  if (!activeProject) return <div style={{ color: D.sub, fontSize: '0.8rem', fontFamily: 'var(--font-mono)', padding: '32px 0', textAlign: 'center', letterSpacing: '0.05em' }}>Select a project in the filter bar above to view site media</div>
  ...
  if (sorted.length === 0) return <div style={{ color: D.sub, fontSize: '0.8rem', fontFamily: 'var(--font-mono)', padding: '32px 0', textAlign: 'center' }}>No media found for this project</div>
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {pageItems.map((item, i) => {
          ...
          return (
            <div key={`${activeProject}-${page}-${i}`} onClick={() => setLightbox(item)} style={{ aspectRatio: '4/3', borderRadius: 8, overflow: 'hidden', cursor: 'pointer', position: 'relative', background: 'rgba(255,255,255,0.04)', border: `1px solid ${D.sub}44`, transition: 'transform 0.2s ease, box-shadow 0.2s ease' }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform = 'scale(1.03)'; (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 24px rgba(0,0,0,0.5), 0 0 0 1px ${D.amber}44` }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.transform = 'scale(1)'; (e.currentTarget as HTMLDivElement).style.boxShadow = 'none' }}>
              {isVideo ? <video src={item.file} muted playsInline preload="none" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} /> : <img src={item.file} alt="" loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
              {isVideo && <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}><div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width={14} height={14} viewBox="0 0 24 24" fill={D.amber}><polygon points="5,3 19,12 5,21"/></svg></div></div>}
              <div style={{ position: 'absolute', top: 6, left: 6, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', borderRadius: 4, padding: '2px 7px', fontSize: 10, color: D.muted, fontFamily: 'var(--font-mono)' }}>{page * PAGE_SIZE + i + 1}</div>
            </div>
          )
        })}
      </div>
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{ background: page === 0 ? 'transparent' : D.panel, color: page === 0 ? D.sub : D.amber, border: `1px solid ${page === 0 ? D.sub : D.amber}`, borderRadius: 6, padding: '7px 18px', fontSize: 12, cursor: page === 0 ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-mono)', letterSpacing: 1, transition: 'all 0.2s', boxShadow: page === 0 ? 'none' : SH_RAISED }}>‹ Prev</button>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            {Array.from({ length: totalPages }, (_, i) => i).filter(i => i === 0 || i === totalPages - 1 || Math.abs(i - page) <= 2).map((i, idx, arr) => (
              <>
                ...
                <button key={i} onClick={() => setPage(i)} style={{ width: 30, height: 30, borderRadius: 6, border: `1px solid ${i === page ? D.amber : D.sub}`, background: i === page ? D.amber : 'transparent', color: i === page ? '#000' : D.muted, fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-mono)', fontWeight: i === page ? 700 : 400, transition: 'all 0.2s', boxShadow: i === page ? SH_RAISED : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</button>
              </>
            ))}
          </div>
          <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} style={{ background: page === totalPages - 1 ? 'transparent' : D.panel, color: page === totalPages - 1 ? D.sub : D.amber, border: `1px solid ${page === totalPages - 1 ? D.sub : D.amber}`, borderRadius: 6, padding: '7px 18px', fontSize: 12, cursor: page === totalPages - 1 ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-mono)', letterSpacing: 1, transition: 'all 0.2s', boxShadow: page === totalPages - 1 ? 'none' : SH_RAISED }}>Next ›</button>
        </div>
      )}
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, backdropFilter: 'blur(8px)', animation: 'fadeIn 0.2s ease' }}>
          {lightbox.media_type === 'video' ? <video key={lightbox.file} src={lightbox.file} autoPlay controls playsInline onClick={e => e.stopPropagation()} style={{ maxWidth: '90vw', maxHeight: '88vh', borderRadius: 10, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }} /> : <img src={lightbox.file} alt="" onClick={e => e.stopPropagation()} style={{ maxWidth: '90vw', maxHeight: '88vh', objectFit: 'contain', borderRadius: 10, boxShadow: '0 32px 80px rgba(0,0,0,0.8)' }} />}
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
        <thead><tr>{['Date','Project','Section','Category','Type','Reporter','Status'].map(h => <th key={h} style={{ padding: '10px 14px', textAlign: 'left', color: D.sub, fontFamily: 'var(--font-mono)', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', borderBottom: `1px solid ${D.border}`, fontWeight: 500, whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead>
        <tbody>
          {reports.map((r, i) => {
            const sc = STATUS_COLOR[r.activity_status] || D.sub
            ...
            return (
              <tr key={r.id} style={{ borderBottom: `1px solid rgba(255,255,255,0.04)`, opacity: 0, animation: `fadeIn 0.35s ease ${i * 0.05}s forwards` }} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.03)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                ...
                <td style={{ padding: '11px 14px', color: D.muted, whiteSpace: 'nowrap' }}><span style={{ background: `${CAT_COLORS[0]}15`, color: CAT_COLORS[0], border: `1px solid ${CAT_COLORS[0]}30`, padding: '2px 8px', borderRadius: 4, fontFamily: 'var(--font-mono)', fontSize: '0.65rem' }}>{r.activity_category || '—'}</span></td>
                ...
                <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>{r.activity_status ? <span style={{ background: `${sc}12`, color: sc, border: `1px solid ${sc}30`, padding: '2px 8px', borderRadius: 4, fontFamily: 'var(--font-mono)', fontSize: '0.62rem' }}>{r.activity_status}</span> : <span style={{ color: D.sub }}>—</span>}</td>
              </tr>
            )
          })}
        </tbody>
...
function RevealOnScroll({ children, delay = 0, style: st }: { children: React.ReactNode; delay?: number; style?: React.CSSProperties }) {
  ...
  return <div ref={ref} style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(28px)', transition: `opacity 0.55s ease ${delay}ms, transform 0.55s cubic-bezier(0.16,1,0.3,1) ${delay}ms`, ...st }}>{children}</div>
}
...
function Panel({ children, title, action, style: st }: { children: React.ReactNode; title: string; action?: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{ background: '#1e1e22', borderRadius: 12, padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 18, boxShadow: `${SH_WELL}, 0 1px 0 rgba(255,255,255,0.04)`, animation: 'borderFlicker 14s ease-in-out infinite', ...st }}>
...
/* ── Filter bar with local-state chainage inputs ───────────── */
function FilterBar({ data, onFilter }: { data: DashData; onFilter: (key: string, val: string) => void }) {
  const active = data.activeFilters
  const hasFilters = !!(active.filterCategory || active.filterProject || active.filterDateFrom || active.filterDateTo || active.filterChFrom || active.filterChTo)

  // Local state for chainage — only fires when both are filled + Enter/blur
  const [chFrom, setChFrom] = useState(active.filterChFrom || '')
  const [chTo,   setChTo]   = useState(active.filterChTo   || '')

  // Sync local state if URL params are cleared externally
  useEffect(() => { setChFrom(active.filterChFrom || '') }, [active.filterChFrom])
  useEffect(() => { setChTo(active.filterChTo || '')     }, [active.filterChTo])

  const applyChFilter = () => {
    const from = chFrom.trim()
    const to   = chTo.trim()
    if (from && to && !isNaN(Number(from)) && !isNaN(Number(to)) && Number(to) > Number(from)) {
      onFilter('ch_from', from)
      onFilter('ch_to',   to)
    }
  }

  const selectStyle: React.CSSProperties = { background: D.bg, color: D.text, border: `1px solid ${D.sub}`, borderRadius: 6, padding: '6px 10px', fontSize: 12, fontFamily: 'var(--font-mono)', cursor: 'pointer', minWidth: 160, boxShadow: SH_WELL }
  const inputStyle: React.CSSProperties  = { ...selectStyle, minWidth: 130 }
  const labelStyle: React.CSSProperties  = { fontSize: 10, color: D.muted, letterSpacing: 1, fontFamily: 'var(--font-mono)', textTransform: 'uppercase', marginBottom: 4 }
  ...
  return (
    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end', padding: '14px 18px', background: D.panel, boxShadow: SH_WELL, borderRadius: 10, marginBottom: 20 }}>
      ...
      {/* Chainage — local state, fires on Enter or blur when BOTH filled */}
      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={labelStyle}>Chainage From</div>
        <input
          type='number' placeholder='e.g. 20000'
          value={chFrom}
          onChange={e => setChFrom(e.target.value)}
          onBlur={applyChFilter}
          onKeyDown={e => { if (e.key === 'Enter') applyChFilter() }}
          style={{ ...inputStyle, minWidth: 110 }}
        />
      </div>

      <div style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={labelStyle}>Chainage To</div>
        <input
      ...
      {/* Helper hint */}
      {(chFrom || chTo) && !(chFrom && chTo) && (
        <div style={{ alignSelf: 'flex-end', fontSize: 10, color: D.amber, fontFamily: 'var(--font-mono)', opacity: 0.7 }}>
          Enter both values to apply
        </div>
      )}

      {hasFilters && (
        <button onClick={() => { setChFrom(''); setChTo(''); onFilter('__clear__', '') }}
          style={{ background: 'transparent', color: D.amber, border: `1px solid ${D.amber}`, borderRadius: 6, padding: '6px 16px', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-mono)', letterSpacing: 1, alignSelf: 'flex-end' }}>
          ✕ Clear
        </button>
      )}
...
function DashboardSkeleton() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14 }}>{[0,1,2,3,4].map(i => <SkeletonBlock key={i} h={100} style={{ animationDelay: `${i * 0.08}s` }} />)}</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 14 }}><SkeletonBlock h={240} /><SkeletonBlock h={240} style={{ animationDelay: '0.1s' }} /></div>
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 14 }}><SkeletonBlock h={200} style={{ animationDelay: '0.05s' }} /><SkeletonBlock h={200} style={{ animationDelay: '0.15s' }} /></div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}><SkeletonBlock h={180} /><SkeletonBlock h={180} style={{ animationDelay: '0.1s' }} /></div>
      <SkeletonBlock h={160} />
...
function DashboardPageInner() {
  ...
  function handleFilter(key: string, val: string) {
    ...
    if (key === '__clear__') {
      params.delete('category'); params.delete('project')
      params.delete('date_from'); params.delete('date_to')
      params.delete('ch_from'); params.delete('ch_to')
    } else if (val) {
      params.set(key, val)
    } else {
      params.delete(key)
          ...
          <div style={{ flex: 1, overflow: 'hidden', maskImage: 'linear-gradient(90deg, transparent, black 8%, black 92%, transparent)', WebkitMaskImage: 'linear-gradient(90deg, transparent, black 8%, black 92%, transparent)' }}>
            <div style={{ display: 'flex', gap: 48, whiteSpace: 'nowrap', animation: 'dataTickerScroll 32s linear infinite', width: 'max-content' }}>
              {[
                ...
                <span key={i} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.52rem', color: item.startsWith('FILTERED') || item.startsWith('CH:') ? D.amber : D.sub, letterSpacing: '0.1em' }}>
      ...
      <div style={{ padding: '28px 32px 60px', maxWidth: 1440, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        {!loading && data && (<>
          ...
          {data.recentReports.length > 0 && (
            <RevealOnScroll>
              <Panel title="Recent Activity Reports"><ReportFeed reports={data.recentReports} /></Panel>
            </RevealOnScroll>
          )}
        </>)}
      </div>
