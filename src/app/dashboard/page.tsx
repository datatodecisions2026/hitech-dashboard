...
/* Was a permanent sticky left column (2026-09-19) — flagged as reserving a
   fixed-width column that was mostly empty space, then turned into a
   floating overlay (2026-09-24). Its "Refine the View / Narrows every
   chart…" header was flagged as unnecessary chrome — the "Filters" toggle
   button that opens this already says what it is, so the header here is
   now just a slim icon+label strip, and every field gets its own small
   icon for faster visual scanning instead of relying on plain text labels
   alone. Same filter state/logic throughout — only presentation changed. */
  ...
  return (
    <aside className="filter-rail" style={{
      ...
      <div style={{ padding: '16px 16px 18px', display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
        ...
        {row(<FIconPin />, 'Section', <select value={active.filterSection || ''} onChange={e => onFilter('section', e.target.value)} style={{ ...field, cursor: 'pointer' }}><option value="">All Sections</option>{data.filterOptions.sections.map(s => <option key={s} value={s}>{s}</option>)}</select>)}
