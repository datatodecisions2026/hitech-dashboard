...
// Ranked lists here can run past a dozen rows — capping the default view
// keeps a long tail from dwarfing whatever shorter card (a donut, a small
// legend) happens to sit next to it in the same grid row, which otherwise
// reads as a lopsided, jagged-bottomed row even once cards are no longer
// force-stretched to match (see the 2026-09-24 (7) changelog entry — that
// fix stopped the *forced* stretch, this addresses the resulting *height
// mismatch* it exposed).
        ...
        {data && (
          <div className="dash-main" style={{ opacity: loading ? 0.6 : 1, pointerEvents: loading ? 'none' : 'auto', transition: `opacity 0.25s ${EASE}` }}>
            ...
            {/* floating filter toggle — was a permanent sticky left column
               ...
               match /dashboard, same instant-onChange behaviour as before,
