...
src/
  app/
    ...
    road-assets/page.tsx    # Calabar/Ogun/Kebbi as-built asset-layer coverage page, self-contained — own Panel/KPICard/Reveal. Overview + Timeline (gauges/bar chart/histogram/Gantt) + By Chainage (real per-segment coverage bar + fabricated itemized completion log, entity/date/chainage filters) tabs — real data mixed with a fabricated planned schedule throughout, not visually distinguished in the UI as of 2026-08-01 (3). See 2026-08-01 changelog entries
    api/
      auth/
      ...
      road-assets/route.ts  # GET  — as-built asset-layer coverage/gap stats for road_assets (session-guarded). See 2026-08-01 changelog
  components/
    ...
    SideNav.tsx             # 64px icon rail (Dashboard/Progress/Machines/Personnel/Streetlights/Road Assets), sticky below header, hidden on /login and <640px
...
scripts/                    # Node maintenance/verification scripts (run manually, not part of the app) — backfill-chainage.mjs, check-ranges.mjs, click-filter-check.mjs, mint-session.mjs, verify-hr-filters.mjs, visual-check.mjs, road-assets-migration.sql (one-off SQL for /road-assets — see 2026-08-01 changelog)
...
Returns as-built asset-layer coverage/gap stats for `/road-assets`, backing Calabar/Ogun/Kebbi — a different data model from `/api/progress` (no planned dates; "coverage" is derived from station presence vs. total road length). Session-guarded.
...
5. `GET /api/dashboard`, `GET /api/progress`, `GET /api/streetlights`, `GET /api/road-design`, and `GET /api/road-assets` also guard with a session check — return 401 if unauthenticated. `GET /api/map` does **not** guard — it's fetched client-side by `HitechMap` and returns no user-identifying data, but keep that in mind if its response shape ever changes
...
**Files changed:** `src/app/api/road-assets/route.ts`, `src/app/road-assets/page.tsx`
...
### 2026-08-01 (2) — `/road-assets` Timeline tab: fabricated, clearly-labeled planned-vs-actual-vs-gap, daily/monthly/yearly

**Files changed:** `src/app/api/road-assets/route.ts`, `src/app/road-assets/page.tsx`

**What changed:**
- Follow-up ask in the same session that built `/road-assets`: the user wanted it to also show progress over time — daily/monthly/yearly, planned vs actual vs gap — the same kind of view `/progress` (Coastal Road) already has via its Gantt chart and progress curve. They referred to `/progress` as "the boss page."
...
### 2026-08-01 — New `/road-assets` page: Calabar/Ogun/Kebbi as-built coverage, and a 4-round journey to make a 7.27M-row query actually perform

**Files changed:** `src/app/road-assets/page.tsx` (new), `src/app/api/road-assets/route.ts` (new), `scripts/road-assets-migration.sql` (new), `src/components/SideNav.tsx`, `src/components/DashHeader.tsx`
